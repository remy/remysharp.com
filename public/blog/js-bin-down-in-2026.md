...
January 27th I got an email notification that JS Bin had become unavailable. Then next day real life human beings were asking what's going on. By 11pm on the 30th the last of the issues were resolved.
...
For the last 5 ish/maybe years or more, JS Bin has run in a fairly automated maintenance mode. There's usually little flurries of wobbles that need my attention every 3/6 months.
...
You can see from the last 11 years of status checks, there's been outages but nothing quite like what happened this time around:

![A chart starting from 2014 with spots throughout showing when JS Bin was down](/images/jsbin-status-chart.avif)

The larger hours when the server goes down (as I found out recently) is memory running out on the machine, and the machine will respond by pretty much collapsing. The result of which is that I typically can't connect to the machine via ssh and restore it. It usually requires a forced reboot via the AWS console.
...
JS Bin was rebooting and immediately locking up. So this tells me there's something externally pushing pressure on the machine that's not easing up.

The only option I have to hand is to shutdown the machine entirely for an intentional hour or so to let whatever is clawing at the door to go away for a bit.

When I eventually looked at the cloud watch logs, it was clear there was something absolutely smashing at the walls. The amount of inbound network traffic is unprecedented for jsbin. From the chart below, it shows normal usage, a trickle of network inbound traffic by comparison to what I was now seeing.

The dips that happen after the vertical wall of monitoring is the machine giving up and being unresponsive. From the chart, I can see that even the short outage wasn't enough to get this beast off jsbin's back.
...
That wouldn't stop things getting sluggish on the machine but it would mean that I could continue to diagnose even whilst the traffic was bombarding the server (albeit with a very slow terminal responding).
...
As it happens, jsbin had been running on node 7. Yes, not even node 8 and on a "non-stable" node for, I imagine, well over a decade.
...
After more proving than I'd like, eventually it turned out that the version of node was in my terminal and at some point during the nginx tuning, Chatgpt had helped itself to what was on the screen.
...
A few times in the past, but I was worried that the configuration adjustment and changes I'd need would be either complicated or would cause some other issue.
...
Props where due, putting CloudFlare in front of JS Bin was relatively easy. CloudFlare detected _most_ of the domains and where they pointed (the important ones at this stage) and it was a matter of swapping over the name servers from pointing to AWS's Route 53 to CloudFlare's own name servers.
...
This is where using the LLM cost me time a problems. I think I was too buried in the thick of things otherwise I might have caught it earlier (and not at the end of the process).
...
This would mean adding this line to each `server {}` block - this would say "If the request doesn't have the CloudFlare header, then don't let it through":
...
This config was all well and good, but traffic was still coming in. This change would mean that nginx would consider the traffic than ignoring it entirely (_foreshadowing_).

When I ran this command, it would list the connections, and I could _still_ see non-CloudFlare requests being able to complete request (at this specific point in time, I was in a frantic cycle of "try a change - fail - worry a bit - ask LLM - repeat"):

```
ss -tan state established '( sport = :443 )'
```

The next change that I put in place was to drop the request if the IP didn't match (whereas before the change was looking for a CloudFlare specific header). Looking back as I write this, I'm not sure why the header method didn't work, but I was sure to add more petrol onto the fire:
...
What I didn't realise at this point, and took another 24 hours to figure out, is that the first CloudFlare nginx based change would say "if the IP is CloudFlare, then set the $remote_addr to the original request". Then this code says "if the IP is NOT CloudFlare don't let them through".
...
This was done at two levels - just for _belt-and-braces_ approach:
...
I naively thought 520 was like a 503 (the entire server is failing to respond to CloudFlare) or a 504 (gateway timeout - usually when node doesn't come back to nginx or CloudFlare), but it's not. It's more like "CloudFlare made a request and the response is incompatible".

The only real clue I had was that port 80, plain http wasn't affected. A [helpful comment from @robobuljan](https://github.com/jsbin/jsbin/issues/3583#issuecomment-3823361117) showed that it was the https version causing issues:
...
That's a lot on TLSv1.3, but I wondered how old v1.3 was given how old my rickety machine was. So I went back to my nginx config and found this line, repeated against each server block that ran SSL:
...
The short answer is yes, but it's pretty well hidden.
