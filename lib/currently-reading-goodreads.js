...
const ignore = new Set([
  ...
  'spectrum-machine-language-for-the-absolute-beginner-2-retro-reproductions',
  'redwall-the-first-book-in-the-beloved-bestselling-saga',
  // 'dark-matter',
]);
...
read: ${Array.isArray(data.read)
      ? JSON.stringify(data.read.sort())
      : data.read.toJSON()
    }
start: ${Array.isArray(data.start)
      ? JSON.stringify(data.start.sort())
      : data.start.toJSON()
    }
rating: ${data.rating}
---
...
(async () => {
  try {
    const response = await fetch(
      'https://www.goodreads.com/review/list/63680802-remy-sharp?per_page=100&shelf=currently-reading&utf8=%E2%9C%93',
      {
        headers: {
    ...
    console.log('updating currently reading');
...
(async () => {
  try {
    ...
    const res = $('.bookalike')
      .map((i, tr) => {
        ...
        return {
          ...
          rating: el.find('.rating .stars .on').length,
          read,
          start,
    ...
    const slugs = res.reduce(
      (/** @type Map */ acc, curr) =>
        acc.set(slug(curr.seriesEntry || curr.title), curr),
      new Map()
    );
    ...
    for (let [slug, book] of slugs.entries()) {
      ...
      if (!reviews.includes(slug) && !reviews.includes(book.titleSlug)) {
        if (!ignore.has(slug)) {
          try {
...
function transform(source) {
  ...
  return source.map(({ title, author_name, link, book, ...rest }) => {
    return {
      title,
      author: author_name,
