...
const currentlyReading = require('../public/books/_currently.json');
const { default: bm25, buildBM25 } = require('./bm25.mjs');
...
function newDate(d) {
  const date = new Date(firstDate(d));
...
const firstDate = (date) => {
  if (Array.isArray(date)) {
    return date.sort(simpleSort)[0].toJSON();
  }
...
module.exports = async (content) => {
  ...
  const clippings = require('../public/books/_clippings.json');
  const books = content
    ...
    .map((book) => {
      ...
      if (clippings[book.slug]) {
        book.data.clippings = clippings[book.slug].highlights;
      }
    ...
    .map((post) => {
      if (!post.date) {
        try {
          ...
          console.log('error', post.inputPath);
          console.log(e);
        }
      }
      return post;
  ...
  blog.forEach((post) => {
    ...
    if (likes[post.slug]) {
      ...
      let images = likes[post.slug]
        .map(
          (_) =>
            `<a class="reset" href="${_.author.url}"><img width="32" title="${_.author.name}" height="32" onerror="this.src='/images/no-user.svg'" loading="lazy" src="${_.author.photo}" alt="${_.author.name}"></a>`
        )
        .join('');
  ...
  const postCountsByYear = blog.reduce((acc, post) => {
  ...
  collections.social = await social();
  collections.squash = bm25;
  collections.bm25 = buildBM25(collections.all)
  collections.withAds = require('./with-ads');
