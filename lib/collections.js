...
const sortBy = (field, secondary) => {
  ...
  return (a, b) => (a.data[field] < b.data[field] ? 1 : -1);
};
...
module.exports = async (content) => {
  ...
  content.forEach((post) => {
    ...
    post.modified = post.data.modified = new Date(
      dates[post.inputPath].modified
    );
    if (!post.data.date) {
  ...
  const devlog = content
    ...
    .map((post) => {
      ...
      post.data.date = post.date = post.data.modified = date;
