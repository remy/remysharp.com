...
module.exports = async (content) => {
  ...
  content.forEach((post) => {
    post.data.modified = new Date(dates[post.inputPath]);
  });
  ...
  const til = content
    ...
    .map(add('type', 'til'))
    .map((post) => {
      if (!post.input) console.log(post);

      let match = post.input.match(/^#(?:[^#]\s*(.*))$/m);
      if (!match) console.log('match fail', post.input);
  ...
  const devlog = content
    ...
    .map((post) => {
      ...
      parts.pop();
      post.data.date = post.date = post.data.modified;
  ...
  const collections = {
    all: content,
    posts: blog.concat(newsletters, links, devlog, til).sort(sortBy('date')),
    newsletters,
    blog,
    links,
    devlog,
